...
from bibscraper.utils.bibtex import (
...
class WebScraper:
    ...
    def scrape(self) -> Dict[str, str]:
        ...
        for entry in entries:
            # Use normalized version only for matching
            norm_entry = normalize_text(entry)
            key = extract_citation_key(norm_entry)
            if key:
                ...
                # Store the original entry instead of the normalized one
                clean_entries[key] = entry
